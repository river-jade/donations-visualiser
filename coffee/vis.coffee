...
Network = () ->
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.amount))
  ...
  # attr is value stored in node, like 'amount'
  ...
  sortedArtists = (nodes,links) ->
    ...
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.amount] ?= 0
        counts[l.source.amount] += 1
        counts[l.target.amount] ?= 0
        counts[l.target.amount] += 1
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.amount))
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.amount)).darker().toString()
