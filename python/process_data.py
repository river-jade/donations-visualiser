...
#Outputs:
FILE_TEMP_DATA = "OUTPUT/temp_data.json"
FILE_ALL_DATA = "OUTPUT/all_data.json"
...
# Finds unique items and prevents duplicates
def discover_unique_items_from_csv(item_description, csv_data_file, duplicate_comparator, parent_identifier_label):
    ...
    for index, item in enumerate(all_items):
        ...
        # If parent identifier column existsl compare all rows in search of the shortest identifying party name
...
def discover_receipt_types(csv_data_file):
    ...
    for receipt in receipt_data_dict:
        if 'Receipt Type' in receipt and receipt['Receipt Type'] not in receipt_types:
            if receipt['Receipt Type'] != "": # prevent blank receipt type from being added
...
def munge_receipt_type(given_receipt_type, payer_name, all_public_entities):
    # TODO: This needs to come from some entity/alias data, this is getting rediculous (dirty data)
    client = payer_name.lower()

    
    for index, public_entity in enumerate(all_public_entities):
        if public_entity['Name'] == client:
            return 'Public Funding'
        
        for alias in public_entity['Aliases']:
            if client == alias.lower():
                return 'Public Funding'

    if 'electoral comission' in client: # catch-all because there is SO much dirty data
        return 'Public Funding'
    elif 'electoral commission' in client: # like you wouldn't believe
        return 'Public Funding'
...
def process_detailed_receipts(csv_data_file, parties, third_parties, campaigners, associated_entities, all_public_entities):
    ...
    for receipt in receipt_data_dict:
        ...
        receipt_type = munge_receipt_type(receipt['Receipt Type'], payer_name, all_public_entities)
...
def transform_receipts_to_old_format(all_detailed_receipts, receipt_entities, receipt_types):
    old_format_receipts = []
    non_party_receipts = 0
    
    for full_receipt in all_detailed_receipts:
        if (full_receipt['Recipient'][1] == "Party"):
            receipt_donor_entity = -1
            
            for index, entity in enumerate(receipt_entities):
                if entity['Name'].lower() == full_receipt['Entity'].lower():
                    receipt_donor_entity = index

            receipt_type = -1
            
            for index, receipt_t in enumerate(receipt_types):
                if full_receipt['Type'] == receipt_t:
                    receipt_type = index
                    # !TODO: Fallback to useful receipt type (ie. not neg 1, or handle in frontend appropriately)
              
            receipt = {
                        'Amount': full_receipt['Amount'],
                        'Type': receipt_type, 
                        'Party': full_receipt['Recipient'][0], 
                        'Year': full_receipt['Year'].split('-')[0], 
                        'Entity': receipt_donor_entity, 
                    }
            old_format_receipts.append(receipt)
        else:
            non_party_receipts +=1
    
    print(f"Total Non-Party Receipts: {non_party_receipts}.")

    return old_format_receipts
                   
if __name__ == "__main__":
    all_public_entities = discover_public_entities()
    all_parties = discover_parties()
    all_third_parties = discover_unique_items_from_csv("third parties", f"{DATA_DIR}{USEFUL_DATA['THIRD_PARTY_RETURNS']}",'Name', '')
    # all_donors = discover_unique_items_from_csv("donors", f"{DATA_DIR}{USEFUL_DATA['DONOR_RETURNS']}",'Name', '') #! Not used
    all_campaigners = discover_unique_items_from_csv("political campaigners", f"{DATA_DIR}{USEFUL_DATA['CAMPAIGNER_RETURNS']}",'Name', '')
    all_associated_entities = discover_unique_items_from_csv("associated entities", f"{DATA_DIR}{USEFUL_DATA['ASSOCIATED_RETURNS']}",'Name', 'Lodged on behalf of')
    ...
    all_parties_enumerated = enumerate_parties(all_parties) #Update party list to link aliases and enumerate
    # unknown_receipt_entities = clean_receipt_entities_with_known(all_receipt_entities, all_parties_enumerated, all_third_parties, all_campaigners, all_associated_entities, all_public_entities) #! Not used

    # For old Data format - produce enumerated list of parties (not aliases) - use all_parties_enumerated to match receipts given aliases
    parties_reduced_list = []
    for party in all_parties_enumerated:
        parties_reduced_list.append(party["Party"])
    
    all_detailed_receipts = process_detailed_receipts(f"{DATA_DIR}{USEFUL_DATA['DETAILED_RECEIPTS']}", all_parties_enumerated, all_third_parties, all_campaigners, all_associated_entities, all_public_entities)
    all_detailed_receipts = transform_receipts_to_old_format(all_detailed_receipts, all_receipt_entities, all_receipt_types)

    all_data = {
        'parties': parties_reduced_list, # Parent Party List
        'receipt_types': all_receipt_types, # All Receipt Types Found
        'receipts': all_detailed_receipts,
        'entities': all_receipt_entities #TODO: Possibly need to remove public entities?
    }
